function Invoke-GPORevContent {
    ...
    if ($GPOPath) {
        Write-Verbose "Invoke-GPORevContent - Reading GPOs from $GPOPath"
        if (Test-Path -LiteralPath $GPOPath) {
            ...
            [Array] $GPOs = foreach ($File in $GPOFiles) {
                if ($File.Name -ne 'GPOList.xml') {
                    try {
                        ...
                        Write-Warning "Invoke-GPORevContent - Couldn't process $($File.FullName) error: $($_.Exception.message)"
                        continue
                    }
                    [PSCustomObject] @{
            ...
            Write-Warning "Invoke-GPORevContent - $GPOPath doesn't exists."
            return
        }
    } else {
        Write-Verbose "Invoke-GPORevContent - Query AD for GPOs"
        [Array] $GPOs = Get-GPORevAD -Forest $Forest -IncludeDomains $IncludeDomains -ExcludeDomains $ExcludeDomains -ExtendedForestInformation $ExtendedForestInformation
    }
    ...
    Write-Verbose "Invoke-GPORevContent - Loading GPO Report to Categories"
    $CountGPO = 0
    [Array] $GPOCategories = foreach ($GPO in $GPOs) {
        $CountGPO++
        Write-Verbose "Invoke-GPORevContent - Processing [$CountGPO/$($GPOs.Count)] $($GPO.DisplayName)"
        if ($GPOPath) {
    ...
    if ($Output['CategoriesFull'].Count -gt 0) {
        foreach ($Report in $Type) {
            Write-Verbose "Invoke-GPORevContent - Processing report type $Report"
                ...
                foreach ($GPO in $CategorizedGPO) {
                    ...
                    if ($SingleObject -or ($Report -in $FindRequiredSingle)) {
                        ...
                        if ($Script:GPODitionary[$Report]['CodeSingle']) {
                            #Write-Verbose "Invoke-GPORevContent - Processing $Report single entry mode"
                    ...
                    if (-not $SingleObject) {
                        ...
                        if ($Script:GPODitionary[$Report]['Code']) {
                            #Write-Verbose "Invoke-GPORevContent - Processing $Report multi entry mode"
    ...
    foreach ($Report in $Type) {
        foreach ($ReportType in $Script:GPODitionary[$Report].ByReports) {
            ...
            Write-Verbose "Invoke-GPORevContent - Processing reports based on other report $Report ($FindReport)"
    ...
    if (-not $SkipCleanup) {
        Write-Verbose "Invoke-GPORevContent - Cleaning up output"
        ...
        if ($Output.Reports) {
            ...
            if ($OutputType -eq 'HTML') {
                if (-not $OutputPath) {
                    ...
                    Write-Warning "Invoke-GPORevContent - OutputPath not given. Using $OutputPath"
                }
                Write-Verbose "Invoke-GPORevContent - Generating HTML output"
                New-HTML {
                    ...
                    foreach ($Key in  $Output.Reports.Keys) {
                        New-HTMLTab -Name $Key {
                            Write-Verbose "Invoke-GPORevContent - Generating HTML Table for $Key"
            ...
            Write-Warning "Invoke-GPORevContent - There was no data output for requested types."
        }
    }
}
...
Register-ArgumentCompleter -CommandName Invoke-GPORevContent -ParameterName Type -ScriptBlock $SourcesAutoCompleter