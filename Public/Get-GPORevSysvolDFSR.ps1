function Get-GPORevSysvolDFSR {
    ...
    foreach ($Domain in $ForestInformation.Domains) {
        Write-Verbose "Get-GPORevSysvolDFSR - Processing $Domain"
        foreach ($DC in $ForestInformation.DomainDomainControllers[$Domain]) {
            Write-Verbose "Get-GPORevSysvolDFSR - Processing $Domain \ $($DC.HostName)"
            ...
            if ($DFSR -and $DFSRConfig -and ($DFSR.ReplicatedFolderGuid -eq $DFSRConfig.ReplicatedFolderGuid)) {
                ...
                Write-Warning "Get-GPORevSysvolDFSR - Couldn't process $($DC.HostName). Conditions not met."
            }
        }
    }
}