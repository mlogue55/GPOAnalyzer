function Get-GPORevOwner {
    ...
    Process {
        ...
        if ($GPOName) {
            $getGPORevADSplat['GPOName'] = $GPOName
        } elseif ($GPOGuid) {
            $getGPORevADSplat['GPOGUID'] = $GPOGuid
        }
        $Objects = Get-GPORevAD @getGPORevADSplat
        foreach ($_ in $Objects) {
            $Count++
            Write-Verbose "Get-GPORevOwner - Processing GPO [$Count/$($Objects.Count)]: $($_.DisplayName) from domain: $($_.DomainName)"
