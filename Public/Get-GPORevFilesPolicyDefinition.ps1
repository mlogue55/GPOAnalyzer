function Get-GPORevFilesPolicyDefinition {
    [alias('Get-GPORevFilesPolicyDefinitions')]
    [cmdletbinding()]
    param(
    ...
    foreach ($Domain in $ForestInformation.Domains) {
            ...
            foreach ($FileLanguage in $FilesLanguage) {
                if ($FileLanguage.Extension -eq '.adml') {
                    if ($FilesCache[$Domain][$FileLanguage.BaseName]) {
                        ...
                        #Write-Warning "Get-GPORevFilesPolicyDefinition - File $($FileLanguage.FullName) doesn't have a match."
        ...
        foreach ($e in $err) {
            Write-Warning "Get-GPORevLegacy - $($e.Exception.Message) ($($e.CategoryInfo.Reason))"
        }
        $ExcludeProperty = @(
