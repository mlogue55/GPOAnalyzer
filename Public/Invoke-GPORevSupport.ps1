function Invoke-GPORevSupport {
    ...
    If ($Type -eq 'HTML') {
        ...
        if (-not $Exists) {
            Write-Warning "Invoke-GPORevSupport - PSWriteHTML module is required for HTML functionality. Use XML, Object or NativeHTML option instead."
            return
        }
    }
    ...
    if (-not $Command -and -not $NativeCommand) {
        Write-Warning "Invoke-GPORevSupport - Neither gpresult or Get-GPResultantSetOfPolicy are available. Terminating."
        return
    }
    ...
    #     try {
    #         Write-Verbose "Invoke-GPORevSupport - ComputerName: $($SplatPolicy['Computer']), UserName: $($SplatPolicy['User']), ReportType: $($SplatPolicy['ReportType']), Path: $($SplatPolicy['Path'])"
    ...
    #             Write-Warning "Invoke-GPORevSupport - Are you running as admin? $($_.Exception.Message)"
    #             return
    #         } else {
    #             $ErrorMessage = $($_.Exception.Message).Replace([Environment]::NewLine, ' ')
    #             Write-Warning "Invoke-GPORevSupport - Error: $ErrorMessage"
    #             return
    #         }
    #     }
    # } else {
    ...
    Write-Verbose "Invoke-GPORevSupport - GPResult Arguments: $Arguments"
    Start-Process -NoNewWindow -FilePath 'gpresult' -ArgumentList $Arguments -Wait
    #}
    if ($Type -eq 'NativeHTML') {
        if (-not $PreventShow) {
            Write-Verbose "Invoke-GPORevSupport - Opening up file $($SplatPolicy['Path'])"
    ...
    if ($SplatPolicy.TempXmlPath -and (Test-Path -LiteralPath $SplatPolicy.TempXmlPath)) {
        ...
        if ($PolicyContent) {
            ...
            Write-Warning "Invoke-GPORevSupport - Couldn't load XML file from drive $($SplatPolicy.TempXmlPath). Terminating."
            return
        }
    } else {
        Write-Warning "Invoke-GPORevSupport - Couldn't find XML file on drive $($SplatPolicy.TempXmlPath). Terminating."
        return
    }
    if ($ComputerName) {
        if (-not $PolicyContent.Rsop.'ComputerResults'.EventsDetails) {
            Write-Warning "Invoke-GPORevSupport - Windows Events for Group Policy are missing. Amount of data will be limited. Firewall issue?"
        }
    }
    if (-not $ComputerName) {
    ...
    if ($Type -eq 'XML') {
        ...
        New-GPORevReportConsole -Results $Output -ComputerName $ComputerName
        if ($Type -contains 'Object') {
            $Output
        } elseif ($Type -contains 'HTML') {
            New-GPORevReportHTML -Path $Path -Online:$Online -Open:(-not $PreventShow) -Support $Output
        }
    }
}
