function Get-GPORevPermissionAnalysis {
    [cmdletBinding()]
    param(
    ...
    if (-not $Permissions) {
        $Permissions = Get-GPORevPermission -IncludePermissionType GpoEditDeleteModifySecurity, GpoApply, GpoCustom, GpoRead -ReturnSecurityWhenNoData -IncludeGPOObject -ReturnSingleObject -Forest $Forest -IncludeDomains $IncludeDomains -ExcludeDomains $ExcludeDomains
    }
