function Get-GPORevLink {
    ...
    Begin {
        ...
        if (-not $GPOCache -and -not $Limited) {
            ...
            foreach ($Domain in $ForestInformation.Forest.Domains) {
                Write-Verbose "Get-GPORevLink - Building GPO cache for domain $Domain"
                if ($ForestInformation['QueryServers'][$Domain]) {
                    ...
                    Write-Warning -Message "Get-GPORevLink - Couldn't get query server for $Domain. Skipped."
                }
            }
        }
    }
    Process {
        ...
        if ($AsHashTable -or $Summary) {
            ...
            $Links = Get-GPORevLinkLoop @getGPOLoopSplat
            ...
            Get-GPORevLinkLoop @getGPOLoopSplat
        }
    }
    End {
