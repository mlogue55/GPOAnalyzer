function New-GPORevWMI {
    ...
    if (-not $SkipQueryCheck) {
        try {
            ...
            Write-Warning "New-GPORevWMI - Query error $($_.Exception.Message). Terminating."
            return
        }
    }
    ...
    foreach ($Domain in $ForestInformation.Domains) {
        ...
        [Array] $ExistingWmiFilter = Get-GPORevWMI -ExtendedForestInformation $ForestInformation -IncludeDomains $Domain -Name $Name
        if ($ExistingWmiFilter.Count -eq 0) {
            ...
            try {
                Write-Verbose "New-GPORevWMI - Creating WMI filter $Name in $Domain"
                New-ADObject -Name $GUID -Type "msWMI-Som" -Path $WMIPath -OtherAttributes $OtherAttributes -Server $QueryServer
            } catch {
                Write-Warning "New-GPORevWMI - Creating GPO filter error $($_.Exception.Message). Terminating."
                return
            }
        } else {
            foreach ($_ in $ExistingWmiFilter) {
                Write-Warning "New-GPORevWMI - Skipping creation of GPO because name: $($_.DisplayName) guid: $($_.ID) for $($_.DomainName) already exists."
            }
        }
    }
}