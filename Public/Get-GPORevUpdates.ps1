function Get-GPORevUpdates {
    ...
    Remove-EmptyValue -Hashtable $getGPORevADSplat
    # lets get all the links including sites

    if ($DateRange) {
        Write-Verbose -Message "Get-GPORevUpdates - Get group policies for defined range $DateRange"
    } elseif ($DateFrom -and $DateTo) {
        Write-Verbose -Message "Get-GPORevUpdates - Get group policies for defined range $DateFrom to $DateTo"
    } else {
        Write-Warning -Message "Get-GPORevUpdates - No range is selected. Try again."
        return
    }
    ...
    foreach ($Domain in $ForestInformation.Domains) {
        Write-Verbose -Message "Get-GPORevUpdates - Getting OU's for $Domain"
    ...
    $CurrentCount = 0
    Write-Verbose -Message "Get-GPORevUpdates - Getting GPO information"
    [Array] $GPOs = Get-GPORevAD @getGPORevADSplat
    foreach ($GPO in $GPOs) {
        $CurrentCount++
        Write-Verbose -Message "Get-GPORevUpdates - Processing $($GPO.DisplayName) / $($GPO.DomainName) [$CurrentCount/$($GPOs.Count)]"
