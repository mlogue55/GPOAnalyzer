function Get-GPORevFiles {
    ...
    $GPOList = Get-GPORevAD -ExtendedForestInformation $ForestInformation
    ...
    foreach ($Domain in $ForestInformation.Domains) {
        ...
        Get-ChildItem -Path $Path -ErrorAction SilentlyContinue -Recurse -ErrorVariable err -File -Force | ForEach-Object {
            ...
            if ($Signature) {
                try {
                    ...
                    Write-Warning "Get-GPORevFiles - Error when reading signature: $($_.Exception.Message)"
                }
                if ($DigitalSignature) {
        ...
        foreach ($e in $err) {
            Write-Warning "Get-GPORevFiles - $($e.Exception.Message) ($($e.CategoryInfo.Reason))"
        }
    }
}