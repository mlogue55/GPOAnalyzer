function Get-GPORev {
    ...
    Begin {
        if (-not $ADAdministrativeGroups) {
            Write-Verbose "Get-GPORev - Getting ADAdministrativeGroups"
        ...
        if ($ExcludeGroupPolicies) {
            ...
            foreach ($GroupPolicy in $ExExecution) {
                if ($GroupPolicy -is [string]) {
                    ...
                    Write-Warning "Get-GPORev - Exclusion takes only Group Policy Name as string, or as hashtable with domain name @{ Name = 'Group Policy Name'; DomainName = 'Domain' }."
                    continue
                }
            }
        }
        if ($OwnerOnly -or $PermissionsOnly -and $Type) {
            Write-Warning "Get-GPORev - Using PermissionOnly or OwnerOnly with Type is not supported. "
        }
        if (-not $GPOPath) {
            ...
            $LinksSummaryCache = Get-GPORevLink -AsHashTable -Summary -Forest $Forest -IncludeDomains $IncludeDomains -ExcludeDomains $ExcludeDomains -ExtendedForestInformation $ExtendedForestInformation
        }
    }
    Process {
        if (-not $GPOPath) {
            foreach ($Domain in $ForestInformation.Domains) {
                ...
                foreach ($GPO in $GroupPolicies) {
                    $Count++
                    Write-Verbose "Get-GPORev - Processing [$($GPO.DomainName)]($Count/$($GroupPolicies.Count)) $($_.DisplayName)"
                    if (-not $Limited) {
                        try {
                            ...
                            Write-Warning "Get-GPORev - Failed to get [$($GPO.DomainName)]($Count/$($GroupPolicies.Count)) $($GPO.DisplayName) GPOReport: $($_.Exception.Message). Skipping."
                            continue
                        }
            ...
            foreach ($Path in $GPOPath) {
                Write-Verbose "Get-GPORev - Getting GPO content from XML files"
                ...
                Write-Verbose "Get-GPORev - Finished GPO content from XML files"
            }
        }
    }
    End {
