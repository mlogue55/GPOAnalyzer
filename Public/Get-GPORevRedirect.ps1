function Get-GPORevRedirect {
    ...
    .EXAMPLE
    Get-GPORevRedirect | Format-Table
        ...
        foreach ($Domain in $ForestInformation.Domains) {
            if ($PSCmdlet.ParameterSetName -eq 'GPOGUID') {
                if ($GPOGuid) {
                    ...
                    Write-Warning "Get-GPORevRedirect - GPOGUID parameter is empty. Provide name and try again."
                    continue
                }
                ...
                if ($GPOName) {
                    ...
                    Write-Warning "Get-GPORevRedirect - GPOName parameter is empty. Provide name and try again."
                    continue
                }
            } else {
                $Splat = @{
            ...
            if ($PSBoundParameters.ContainsKey('DateRange')) {
                ...
                if ($DateProperty -contains 'WhenChanged' -and $DateProperty -contains 'WhenCreated') {
                    ...
                    Write-Warning -Message "Get-GPORevRedirect - DateProperty parameter is empty. Provide name and try again."
                    continue
                }
            ...
            Write-Verbose -Message "Get-GPORevRedirect - Searching domain $Domain with filter $($Splat['Filter'])"
            ...
            foreach ($Object in $Objects) {
                ...
                if (($GUID).Length -ne 36) {
                    Write-Warning "Get-GPORevRedirect - GPO GUID ($($($GUID.Replace("`n",' ')))) is incorrect. Skipping $($Object.DisplayName) / Domain: $($DomainCN)"
                } else {
