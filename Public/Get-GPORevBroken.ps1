function Get-GPORevBroken {
    ...
    foreach ($Domain in $ForestInformation.Domains) {
        ...
        Write-Verbose "Get-GPORevBroken - Starting process for $Domain"
        ...
        if ($PoliciesInAD.Count -ge 2) {
            if (-not $VerifyDomainControllers) {
                ...
                foreach ($Server in $ForestInformation['DomainDomainControllers']["$Domain"]) {
                    Write-Verbose "Get-GPORevBroken - Processing $Domain \ $($Server.HostName.Trim())"
            ...
            Write-Warning "Get-GPORevBroken - GPO count for $Domain is less then 2. This is not expected for fully functioning domain. Skipping processing SYSVOL folder."
        }
        $TimeEnd = Stop-TimeLog -Time $TimeLog -Option OneLiner
        Write-Verbose "Get-GPORevBroken - Finishing process for $Domain (Time to process: $TimeEnd)"
    }
}