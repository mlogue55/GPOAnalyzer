function Get-GPORevPermissionConsistency {
    ...
    Process {
        foreach ($Domain in $ForestInformation.Domains) {
            ...
            Write-Verbose "Get-GPORevPermissionConsistency - Starting process for $Domain"
            ...
            $GroupPolicies | ForEach-Object -Process {
                $GPO = $_
                $Count++
                Write-Verbose "Get-GPORevPermissionConsistency - Processing [$($_.DomainName)]($Count/$($GroupPolicies.Count)) $($_.DisplayName)"
                ...
                if (Test-Path -LiteralPath $SysVolpath) {
                    try {
                        ...
                        Write-Warning "Get-GPORevPermissionConsistency - Processing $($GPO.DisplayName) / $($GPO.DomainName) failed to get consistency with error: $($_.Exception.Message)."
                        $IsConsistent = 'Not available'
                    }
                } else {
                    Write-Warning "Get-GPORevPermissionConsistency - Processing $($GPO.DisplayName) / $($GPO.DomainName) failed as path $SysvolPath doesn't exists!"
                    $IsConsistent = $false
                }
            ...
            Write-Verbose "Get-GPORevPermissionConsistency - Finishing process for $Domain (Time to process: $TimeEnd)"
        }
    }
    End {
