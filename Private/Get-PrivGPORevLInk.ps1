    ...
    $ObjectsToProcess | ForEach-Object -Process {
        ...
        if ($Link.Length -gt 10) {
            ...
            if ($Option -eq '0') {
                ...
                if ($Object.GpLink) {
                    Write-Warning "Get-PrivGPORevLink - This should't happen. Please investigate - Option: $Option"
                } else {
                    Write-Warning "Get-PrivGPORevLink - Property GPLink is required to be able to tell if Enabled/Enforced is added. Skipping those settings."
                }
                $Enabled = $null
                $Enforced = $null
            }
            ...
            if ($GPOCache -and -not $Limited) {
                if ($GPOCache[$Search]) {
                    ...
                    Write-Warning "Get-PrivGPORevLink - Couldn't find link $Search in a GPO Cache. Lack of permissions for given GPO? Are you running as admin? Skipping."
                }
            } else {
                $Output['GPODomainDistinguishedName'] = ConvertFrom-DistinguishedName -DistinguishedName $DN -ToDC
                $Output['GPODistinguishedName'] = $DN
